import { z } from "zod";
import { LANGUAGES, PROJECT_MAX_NAME_LEN, PROJECT_MIN_NAME_LEN, STROKE_COLORS, STROKE_WIDTHS, PROJECT_COLLABORATOR_TYPES, PROJECT_COLLABORATOR_ADD_TYPES } from "../../../packages/constants/index.ts";

const languageEnum = z.enum(LANGUAGES);

const strokeColorEnum = z.enum(STROKE_COLORS);

const strokeWidthEnum = z.union(
  STROKE_WIDTHS.map(w => z.literal(w)) as [z.ZodLiteral<2>, z.ZodLiteral<4>, z.ZodLiteral<6>]
...
export const whiteboardTabSchema = z.object({
  whiteboardTabId: z.string().trim().min(1, {message: "Whiteboard tab must have an ID"}),
  paths: z.array(z.object({
    points: z.array(z.tuple([z.number(), z.number()])),
    strokeColor: strokeColorEnum,
  ...
  currPath: z.array(z.tuple([z.number(), z.number()])),
});
...
export const projectCreateClientSchema = z.object({
  name: z
    .string()
    .trim()
    .min(PROJECT_MIN_NAME_LEN, { message: `Project name must be at least ${PROJECT_MIN_NAME_LEN} characters` })
    .max(PROJECT_MAX_NAME_LEN, { message: `Project name must not exceed ${PROJECT_MAX_NAME_LEN} characters` })
});
...
export const projectCreateServerSchema = projectCreateClientSchema.extend({
  ...
  codeTabs: z.array(codeTabSchema).min(1, {message: "At least one code tab is required"}),
  whiteboardTabs: z.array(whiteboardTabSchema).min(1, {message: "At least one whiteboard tab is required"}),
});
...
export const projectCollaboratorAddServerSchema = projectCollaboratorAddClientSchema.extend({
  userId: z
    .string()
    .trim()
